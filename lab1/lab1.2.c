...
void maketexture()
{
	...
	for(int y = 0; y < kTextureSize; y++) {	
		...
		{			
			// Making another substantially different pattern, checkered texture 
			// Using the sine function which is repeating we can input our position and get a value back
			// The constant inside the function can be thought of as the scaling of the checkered pattern
			float sine = sin(x*0.1) + sin(y*0.1);
			if(sine > 0){
				ptex[x][y][0] = 0;
				ptex[x][y][1] = 255;
				ptex[x][y][2] = 0;
